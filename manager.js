...
async function renderStatsTab() {
    ...
    // MIT Stats
    const mitTasks = tasks.filter(t => t.isMIT);
    const mitTotal = mitTasks.length;
    const mitCompleted = mitTasks.filter(t => t.completed || t.mitStatus === 'completed').length;
    const mitCompletionRate = mitTotal > 0 ? Math.round((mitCompleted / mitTotal) * 100) : 0;

    // Event Stats
    const eventCount = tasks.filter(t => t.isEvent).length;
    ...
    container.innerHTML = `
        ...
        <div class="stats-bento-grid">
            ...
            <div class="stats-glass-card stats-momentum-card">
                <div class="stats-section-header">
                    <span class="stats-section-icon">⭐</span>
                    <span class="stats-section-title">MIT Performance</span>
                </div>
                <div class="stats-momentum-value">
                    <span class="stats-momentum-percent">${mitCompletionRate}%</span>
                </div>
                <div class="stats-momentum-label">Success Rate</div>
                <div class="stats-momentum-detail">
                    ${mitCompleted} / ${mitTotal} Most Important Tasks
                </div>
            </div>
            ...
            <div class="stats-glass-card stats-mini-card stats-card-sm">
                <div class="stats-mini-number" style="color:var(--priority-someday)">${eventCount}</div>
                <div class="stats-mini-label">Events</div>
            </div>
...
function createTaskElement(task, options = {}) {
    ...
    // Add event note class
    if (task.isEvent) {
        taskItem.classList.add('event-note');
    }
    ...
    // MIT Star Button
    if (context === 'sidebar' || context === 'grid' || context === 'management') {
        const starBtn = document.createElement('button');
        starBtn.classList.add('mit-star-btn');
        if (task.isMIT) starBtn.classList.add('active');
        starBtn.innerHTML = task.isMIT ? '★' : '☆';
        starBtn.title = task.isMIT ? 'Unmark as MIT' : 'Mark as Most Important Task';
        starBtn.setAttribute('data-task-id', task.id);
        taskItem.appendChild(starBtn);
    }

    // Checkbox (for management and sidebar views)
    // Event notes do not have completion checkboxes
    if (!task.isEvent && (context === 'management' || context === 'sidebar')) {
...
function setupDragAndDropListeners() {
    ...
    plannerContainer.addEventListener('drop', async (event) => {
        ...
        // --- Handle Event Note Cloning (Parking Lot -> Grid) ---
        if (task.isEvent && !currentDragInfo.sourceDay && dropTarget.classList.contains('grid-cell')) {
            const day = dropTarget.dataset.day;
            const blockId = dropTarget.dataset.blockId;

            // Clone the event note
            const newEvent = duplicateTask(task);
            // new ID is generated by duplicateTask
            newEvent.schedule = [{ day, blockId, completed: false }];
            newEvent.title = task.title; // Ensure title is same (note content)
        ...
        if (dropTarget.id === 'unassigned-tasks-list') {
            if (currentDragInfo.sourceDay && currentDragInfo.sourceBlockId) {
                ...
                if (idx > -1) {
                    task.schedule.splice(idx, 1);
                    scheduleChanged = true;

                    // If it was an event note instance on grid, delete it if empty schedule
                    if (task.isEvent && task.schedule.length === 0) {
...
// --- EVENT NOTES LISTENERS ---

function setupEventCreationListeners() {
    ...
    if (addEventBtn && addEventModal) {
        addEventBtn.addEventListener('click', () => {
            // Reset form
            document.getElementById('event-note-content').value = '';
            document.getElementById('event-recurrence').value = '';
            addEventModal.classList.remove('hidden');
        });
    }

    if (closeBtn && addEventModal) {
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            const content = document.getElementById('event-note-content').value.trim();
            const recurrence = document.getElementById('event-recurrence').value || null;

            if (!content) {
                showInfoMessage('Note content is required.', 'error');
                return;
            }

            // Create new Event Note task
            const extraAttrs = {
                isEvent: true,
                status: 'inbox'
            };

            await addNewTask(content, '', 'SOMEDAY', null, 'event', 'TBD', '', recurrence, extraAttrs);

            showInfoMessage('Event note added to Parking Lot!', 'success');
...
function setupMITListeners() {
    document.addEventListener('click', async (e) => {
        if (e.target.matches('.mit-star-btn')) {
            e.stopPropagation(); // Prevent opening details or other actions
            const btn = e.target;
            ...
            if (taskId) {
                ...
                if (task) {
                    ...
                    // If marking as MIT, set mitStatus to pending if it was null/undefined
                    if (task.isMIT && !task.mitStatus) {
                        task.mitStatus = 'pending';
                    }

                    await updateTask(task);
                    renderPage(); // Re-render to update UI state
                }
            }
        }
    });
...
async function checkMITRetrospective() {
    const tasks = await getTasksAsync();
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);

    const pendingPastMITs = tasks.filter(task => {
        if (!task.isMIT) return false;
        if (task.completed) return false; // Already done (via checkbox)
        if (task.mitStatus === 'completed' || task.mitStatus === 'missed') return false; // Already resolved

        // Check if it's from a previous day
        const modDate = new Date(task.lastModified);
        modDate.setHours(0, 0, 0, 0);

        return modDate < todayStart;
    });

    if (pendingPastMITs.length === 0) return;

    // Show Modal
    const modal = document.getElementById('mit-retro-modal');
    const list = document.getElementById('mit-retro-list');
    const closeBtn = document.getElementById('mit-retro-close-btn');

    if (!modal || !list) return;

    list.innerHTML = '';

    pendingPastMITs.forEach(task => {
        const item = document.createElement('div');
        item.classList.add('mit-retro-item');
        item.dataset.taskId = task.id;

        const dateStr = new Date(task.lastModified).toLocaleDateString();

        item.innerHTML = `
            <div class="mit-retro-label">
                <span class="mit-retro-date">From ${dateStr}</span>
                <strong>${task.isEvent ? '📝 ' : ''}${escapeHtml(task.title)}</strong>
            </div>
            <div class="mit-retro-actions">
                <button class="mit-retro-btn yes">✅ Yes</button>
                <button class="mit-retro-btn no">❌ No</button>
            </div>
        `;
        ...
        yesBtn.addEventListener('click', async () => {
            await updateMITStatus(task.id, 'completed');
        ...
        noBtn.addEventListener('click', async () => {
            await updateMITStatus(task.id, 'missed');
            item.classList.add('completed');
            yesBtn.disabled = true;
            noBtn.disabled = true;
        });
...
async function updateMITStatus(taskId, status) {
    const task = await getTaskById(taskId);
    if (task) {
        task.mitStatus = status;
        if (status === 'completed' && !task.isEvent) {
            task.completed = true;
            task.status = 'done';
...
async function checkEventExpiry() {
    const tasks = await getTasksAsync();
    const blocks = await getTimeBlocks();
    const { currentTime } = getCurrentTimeBlockInfo();

    // currentDays is populated by generateDayHeaders (rotated days starting from today)
    // currentDays[0] is today.

    if (!currentDays || currentDays.length === 0) return false;

    let changed = false;
    const tasksToDelete = [];

    tasks.forEach(task => {
        if (task.isEvent && task.schedule && task.schedule.length > 0) {
            // Iterate schedule in reverse to remove items
            for (let i = task.schedule.length - 1; i >= 0; i--) {
                ...
                const dayOffset = currentDays.indexOf(item.day);
                ...
                // If dayOffset < 0, it means the day is not in the current week view.
                // Since this is a rolling planner, this technically shouldn't happen for active tasks unless they are stale.
                // For Events, we assume they are short-lived.

                if (isPast) {
                    task.schedule.splice(i, 1);
                    changed = true;
                }
            }

            // If schedule empty (and it was on grid i.e. had schedule), delete the task instance
...
renderPage = async function() {
    ...
    // Check MIT Retrospective
    await checkMITRetrospective();
};
